-- =====================================================
-- SKRYPT SPRAWDZAJĄCY POPRAWNOŚĆ BAZY DANYCH (POPRAWIONY)
-- =====================================================
...
WITH index_usage AS (
    SELECT
        schemaname,
        relname as tablename,  -- POPRAWKA: relname zamiast tablename
        indexrelname,
        idx_scan,
...
SELECT
    c.relname as "Tabela",  -- POPRAWKA: używamy pg_class
    t.tgname as "Trigger",
...
-- Statystyki tabel - POPRAWKA: relname zamiast tablename
SELECT
    relname as "Tabela",
        ...
        PERFORM (
            SELECT
                ...
                COUNT(CASE WHEN EXISTS (
                    ...
                    WHERE table_name = 'parts' AND column_name = 'cad_2d_binary'
                ) AND cad_2d_binary IS NOT NULL THEN 1 END) as "Z CAD 2D bin",
                pg_size_pretty(
...
-- Alternatywnie - proste zapytanie
SELECT
    'parts' as "Tabela",
    COUNT(*) as "Wszystkie rekordy",
    COUNT(CASE WHEN thumbnail_100 IS NOT NULL THEN 1 END) as "Z thumbnail",
...
WITH recommendations AS (
    -- Vacuum - POPRAWKA: relname zamiast tablename
    SELECT
        1 as priority,
...
-- POPRAWKA: CTE zdefiniowany lokalnie dla tego zapytania
...
SELECT
    CASE
        WHEN EXISTS (SELECT 1 FROM problem_summary_final WHERE exists = true AND severity = 'KRYTYCZNY')
        THEN '❌ KRYTYCZNE PROBLEMY - napraw natychmiast!'
        WHEN EXISTS (SELECT 1 FROM problem_summary_final WHERE exists = true AND severity = 'ŚREDNI')
        THEN '⚠️ PROBLEMY ŚREDNIE - napraw wkrótce'
        WHEN EXISTS (SELECT 1 FROM problem_summary_final WHERE exists = true)
