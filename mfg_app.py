...
class SupabaseManager:
    """Manager for all Supabase operations"""
    ...
    def __init__(self):
        ...
        if not self.url or not self.key:
            raise ValueError("Brak konfiguracji Supabase! Sprawdź plik .env")
    ...
    def delete_order(self, order_id: str) -> bool:
        try:
            ...
            self.client.table('parts').delete().eq('order_id', order_id).execute()
            # Then delete order
    ...
    def create_part(self, part: Part) -> Optional[Dict]:
        try:
            data = {
                ...
                'idx_code': part.idx_code,
                'name': part.name,
                'material': part.material,
                'thickness_mm': part.thickness_mm,
                'qty': part.qty
            }
            response = self.client.table('parts').insert(data).execute()
            return response.data[0] if response.data else None
        except Exception as e:
    ...
    def update_part(self, part_id: str, updates: Dict) -> bool:
        try:
            self.client.table('parts').update(updates).eq('id', part_id).execute()
            return True
        except Exception as e:
    ...
    def delete_part(self, part_id: str) -> bool:
        try:
            self.client.table('parts').delete().eq('id', part_id).execute()
            return True
        except Exception as e:
